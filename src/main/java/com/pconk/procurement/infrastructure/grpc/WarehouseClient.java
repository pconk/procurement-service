...
public class WarehouseClient {
        ...
        private static final Logger LOG = Logger.getLogger(WarehouseClient.class);
        ...
        public Uni<Boolean> increaseStock(List<com.pconk.procurement.domain.entity.PurchaseOrderItem> items) {
                ...
                StockUpdateRequest request = StockUpdateRequest.newBuilder()
                                ...
                                .build();
                LOG.debug("StockUpdateRequest");
                LOG.debug(request);

                // kalo tidak mau kirim ke warehouseService
                // return Uni.createFrom().item(() -> {
                //         return true;
                // });

                // variasi lain untuk jalanin
                // warehouseService.increaseStock(request)
                //         .subscribe().with(
                //         response -> LOG.info("Stock successfully updated in Go service"),
                //         failure -> LOG.errorf("Failed to update stock in Go: %s",
                //                 failure.getMessage()));
